...
public:
  ...
  {
  #if defined(USE_CUDA) && !defined(__CUDA_ARCH__)
    moveNested( internal::getArrayManager().getExecutionSpace(), size, true );
  #else
   ...
   */
  inline
  void moveNested( chai::ExecutionSpace const space, std::ptrdiff_t const size, bool const touch ) const
  {
  #if defined(USE_CUDA)
   ...
   */
  void move( chai::ExecutionSpace const space, bool const touch ) const
  {
  #if defined(USE_CUDA)
    if( m_pointer_record == nullptr ||
        m_capacity == 0 ||
